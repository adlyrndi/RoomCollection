...
const ProductCard: React.FC<ProductCardProps> = ({ image, title, price, ProdName }) => {
  return (
    <div className="w-full flex flex-col">
      <div className="relative w-full h-[760px] bg-white shadow-lg overflow-hidden group">
        <Image
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center p-6 
          ...
          <h3 className="text-5xl font-bold crushed-regular uppercase">{ProdName}</h3>
          <p className="text-md mt-2 uppercase mb-7">{title}</p>
      ...
      <div className="flex justify-between w-full mt-4">
        ...
        {price && <p className="text-md text-gray-900">{price}</p>}
      </div>
    </div>
  );
};
